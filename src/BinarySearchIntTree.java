...
public class BinarySearchIntTree {
	...
	public BinarySearchIntTree( int value ) {
		this.overallRoot = new IntTreeNode(value); //creates node and size +1
		size++;
	}
	...
	public boolean add( int value ) {
		if(overallRoot == null) { //if tree is null
			overallRoot = new IntTreeNode(value); //creates overallRoot
			size++; //size + 1
			return true; //returns true
		}
		IntTreeNode curr = overallRoot; //current temp
		IntTreeNode parent = null; //previous node temp
		while(curr != null) { //loop to traverse
			parent = curr; //sets previous to current
			if(value > parent.data) { //checks see where value goes
				curr = curr.right;
	...
	private static IntTreeNode minNode( IntTreeNode root ) {
		IntTreeNode curr = root; //uses starting root and then checks smallest node from that given tree
		while(curr != null) {
			curr = curr.left;
		}
		return curr;	
	...
	public boolean remove( int num ) {
		...
		IntTreeNode curr = overallRoot; //current temp
		IntTreeNode parent = null; //previous node to current
		...
		if(curr.left != null && curr.right != null) {
			...
			//}
			IntTreeNode minNode = minNode(curr); //creates a temp of minNode of tree
	...
	private static String printInOrder(IntTreeNode root){
		...
		IntTreeNode current, parent; //creates current and previous current node
